...
module Birdv
  module DSL
    module FB
      ...
      def send_story(fb_id, to_send)
        to_send.call(fb_id)
      end

      def send(fb_id, to_send, type='MSG', *args)
        # if lambda, run it! e.g. send(story(args)) 
          if type == 'IMG'
            send_story(fb_id, to_send)
          # else, we're dealing with a hash! e.g send(text("stuff"))
          elsif type == 'MSG'
            send_msg(fb_id, to_send)
          end
      end # send_helper
